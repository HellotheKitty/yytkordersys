...
//每次进入页面之前将所有已收款再打印的订单状态从待结算改成待配送substr($dwdm,0,2) == '34'
...
{
    $restotal = mysql_query("select count(DISTINCT order_mainqt.ddh) as rowcount from b_ry xs,order_mainqt left join order_mxqt on order_mainqt.ddh=order_mxqt.ddh left join order_zh on order_mainqt.ddh=order_zh.ddh and zy<>'订单订金' where order_mainqt.xsbh=xs.bh and (order_mainqt.state='进入生产' or order_mainqt.state='生产完成') and not pczy is null and hdczy is null and zzfy in $dwdmStr ", $conn);
...
<? }
    ...
    if ($_SESSION["FBCW"] == 1 || $_SESSION['FBSD'] == 1) {
        ...
        <? if ($gp == '进入生产'){  ?>
            ...
            <select name = 'producedetail' onchange="form1.submit();">
                <option value="" >更多状态</option>
                <option value="printing" <? if ($gpmore == 'printing') echo "selected"; ?> >打印生产中</option>
                <option value="hding" <? if ($gpmore == 'hding') echo "selected"; ?> >后加工中</option>
            </select>
        <? }
        ...
        ?>
        查找订单：<input type="text" name="fdd" width="15" placeholder="完整或部分的订单号或客户名"/>　　文件名：
        <input type="text" name="filename" width="15" placeholder="备注中的文件名信息"/>　　下单时间：
        <input onClick="WdatePicker();" maxlength="50" class="Wdate" style="cursor:pointer;" type="text" name="rq1" id="rq1" value="<? echo $d1; ?>" size="8" readonly/>～
        <input onClick="WdatePicker();" maxlength="50" class="Wdate" style="cursor:pointer;" type="text" name="rq2" id="rq2" value="<? echo $d2; ?>" size="8" readonly/>
        问题订单:
...
//                                                echo '生产中';
                                    ...
                                    <tr class="td_title" style="height:30px;"
                                        ...
                                        <td dataType="<? if(mysql_result($rs,$i,'state') == '进入生产'){
                                            if(empty(mysql_result($rs,$i,'pczy'))){
                                                echo 'printing';
                                            }elseif(!empty(mysql_result($rs,$i,'pczy')) && empty(mysql_result($rs,$i,'hdczy'))){
                                                echo 'hding';
