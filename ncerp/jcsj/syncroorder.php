...
$dwdm = substr($_SESSION["GDWDM"],0,4);
    ...
    $waixie = mysql_query("select * from order_waixie where ddh= '".$bh."'",$conn);
    $copyddh = mysql_result($waixie,0,'copyddh');

//    $ifexistsmx = mysql_query("select id from order_mxqt where ddh='".$bh."'",$conn);
//    $ifexistsfm = mysql_query("select id from order_mxqt_fm where ddh='".$bh."'",$conn);
//    $ifexistshd = mysql_query("select id from order_mxqt_hd where ddhao='".$bh."'",$conn);
...
//        }
        for($i=0;$i<mysql_num_rows($copymx);$i++){
            ...
            $copyorder[] = mysql_query("insert into order_mxqt (id,ddh,productname,pname,chicun,sl,n1,file1,machine1,paper1,color1,jldw1,dsm1,hzx1,pnum1,sl1,jg1,n2,file2,machine2,paper2,color2,jldw2,dsm2,hzx2,pnum2,sl2,jg2,n3,file3,machine3,paper3,color3,jldw3,dsm3,hzx3,pnum3,sl3,jg3) select 0,'$bh',productname,pname,chicun,sl,n1,file1,machine1,paper1,color1,jldw1,dsm1,hzx1,pnum1,sl1,jg1,n2,file2,machine2,paper2,color2,jldw2,dsm2,hzx2,pnum2,sl2,jg2,n3,file3,machine3,paper3,color3,jldw3,dsm3,hzx3,pnum3,sl3,jg3 from order_mxqt where id='".mysql_result($copymx,$i,'id')."'",$conn);
...
//        update订单金额
        ...
        if(!transaction($copyorder)){
            ...
            echo "<script>alert('委托生产失败！请重新操作');</script>";
        }else{

            echo "<script>window.location.href='NS_new.php?ddh=$bh&from=list'</script>";
        }
