...
//	print_r($_POST);
	...
	$price = $_POST["price"];
	$unit = $_POST["unit"];
	$testrs = mysql_query("select id from price_of_print where khid=$khid and machine='$machine' materialid=$materialid and min=$min and max=$max and dsm='$dsm' and unit='$unit' order by machine,materialid,dsm,unit",$conn);
...
if(substr($dwdm,0,2)=='34'){
	$materialrs = mysql_query("select * from material where zzfy='3405'",$conn);

}else{
	$materialrs = mysql_query("select * from material where zzfy='$dwdm'",$conn);
...
function del(id)
{
	if(confirm("确定删除？")) {
		$.ajax({
			type: "get",
			url: "price_update.php?table=print&id="+id,
			success: function(data){
				if(data > 0){	
					$("#tr"+id).remove();
				}else{
...
<body>
<font size="6"><?echo $_khmc?> 打印价格表</font>
<table>
    <tr>
    	<th width="73px">单/双</th>
    	<th width="138px">机型及颜色</th>
    	<th width="92px">规格</th>
    	<th width="185px">纸张</th>
    	<th width="65px">下限</th>
    	<th width="65px">上限</th>
    	<th width="65px">单位</th>
    	<th width="65px">单价</th>        
