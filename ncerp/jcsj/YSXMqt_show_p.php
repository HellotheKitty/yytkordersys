...
//                            $a1= '拼版号:'.$a1arr[1];
                ...
                if(!empty(mysql_result($rsmx,$i,"jdf1")) && !empty(mysql_result($rs,0,"filefy"))){
            ...
            <td  class="nobottomline" align="center">
                ...
                if($_fmrs && mysql_num_rows($_fmrs)>0) {
                    while($_fmarr = mysql_fetch_assoc($_fmrs)) {
                        ...
                        if($_GET["lx"] == "show"){
                            if($_fmarr['fmczy'] == '')
                                echo '<input type="button" class="finish-btn fm-finish" datatype="fm-' . $_fmarr['id'] . '" value="覆膜完成"/><br/>';
                            else
                                echo '<input type="button" value="已完成" disabled="disabled"/><br/>';
                ...
                <td class="righttd notopline" align="center"><? $_hdrs = mysql_query("select * from order_mxqt_hd where mxid = " . mysql_result($rsmx,$i,"id") . " order by id asc",$conn);
                    if($_hdrs && mysql_num_rows($_hdrs)>0) {
                        while($_arr = mysql_fetch_assoc($_hdrs)) {
                            ...
                            if($_GET["lx"] == "show"){
                                if($_arr['hdczy'] == '')
                                    echo '<input type="button" class="finish-btn hd-finish" datatype="hd-' . $_arr['id'] . '" value="后加工完成"/><br/>';
                                else
                                    echo '<input type="button" value="已完成" disabled="disabled"/><br/>';
                            }
                        }
