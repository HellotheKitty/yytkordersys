...
$sql = "select base_kh.id,order_zh.khmc,sum(jf) from order_zh,base_kh where jf>0 and zy<>'订单结算' and zy<>'订单定金' and locate('充值转支付',zy)=0 and locate('预存赠送',zy)=0 and locate('预存赠送',order_zh.xsbh)=0 and locate('应收款',order_zh.xsbh)=0 and order_zh.khmc=base_kh.khmc and base_kh.gdzk in $dwdmStr and order_zh.fssj>='$d1 00:00:00' and order_zh.fssj<='$d2 23:59:59' and order_zh.khmc like '%$d3%' group by khmc";
...
<form action="" method="post">按日期：<input onClick="WdatePicker();" maxlength="50" class="Wdate" style="cursor:pointer;" type="text" name="rq1" id="rq1" value="<? echo $d1;?>" size="9" readonly />～<input onClick="WdatePicker();" maxlength="50" class="Wdate" style="cursor:pointer;" type="text" name="rq2" id="rq2" value="<? echo $d2;?>" size="9" readonly />&nbsp;客户名称：<input type="text" name="fkhmc" width="15" value="<?echo $d3=="%"?"":$d3;?>"/>&nbsp;&nbsp;

    <? include '../../commonfile/calc_options.php'; ?>

    <input type="submit" name="btn1" value="查询" />&nbsp;&nbsp;<input type="submit" name="btn2" value="导出" /></form>
<body><!--
