...
if(mysql_num_rows($sel_ysk_order) > 0){
...
$sel_ydy_sql = "SELECT DISTINCT m.ddh FROM	order_mxqt x LEFT JOIN order_mainqt m ON x.ddh = m.ddh WHERE m.state = '进入生产' AND ((n1 <> '' AND operator1 IS NOT NULL	AND operator1 <> '') AND ((n2 <> ''	AND operator2 IS NOT NULL	AND operator2 <> '') OR (n2 is NULL)))";
$res_ydy = mysql_query($sel_ydy_sql,$conn);

if(mysql_num_rows($res_ydy)>3){
    mysql_query("update order_mainqt m set m.state='已打印' WHERE m.state='进入生产' AND m.ddh in (SELECT t.ddh from (SELECT DISTINCT m.ddh FROM	order_mxqt x LEFT JOIN order_mainqt m ON x.ddh = m.ddh WHERE m.state = '进入生产' AND ((n1 <> '' AND operator1 IS NOT NULL	AND operator1 <> '') AND ((n2 <> ''	AND operator2 IS NOT NULL	AND operator2 <> '') OR (n2 is NULL)))) as t)");
}
...
//    更多状态
    ...
    if($_GET['trouble_order'] <>''){
        ...
        if($_GET['trouble_order'] == '1'){
            ...
            $troublesql = " and (order_zh.ddh is null and (order_mainqt.state = '订单完成' or order_mainqt.state = '待配送'))";
        }
...
{
    $restotal = mysql_query("select count(DISTINCT order_mainqt.ddh) as rowcount from b_ry xs,order_mainqt left join order_mxqt on order_mainqt.ddh=order_mxqt.ddh left join order_zh on order_mainqt.ddh=order_zh.ddh and zy<>'订单订金' where order_mainqt.xsbh=xs.bh and (order_mainqt.state='进入生产' or order_mainqt.state='已打印' or order_mainqt.state='生产完成') and pendtime is null and zzfy in $dwdmStr",$conn);

    $rowcount = mysql_result($restotal,0,'rowcount');

    include '../commonfile/paging_data.php';

    $rs = mysql_query("select order_mainqt.*,count(order_mxqt.id) mxsl,group_concat(order_mxqt.productname) cpms,xs.xm,sksj from b_ry xs,order_mainqt left join order_mxqt on order_mainqt.ddh=order_mxqt.ddh left join order_zh on order_mainqt.ddh=order_zh.ddh and zy<>'订单订金' where order_mainqt.xsbh=xs.bh and (order_mainqt.state='进入生产' or order_mainqt.state='已打印' or order_mainqt.state='生产完成') and pendtime is null and zzfy in $dwdmStr group by order_mainqt.ddh order by ddate desc limit $startrow ,$pagenum", $conn);

}

elseif ($_SESSION["FBHD"] == 1) //生产后加工
{
    $restotal = mysql_query("select count(DISTINCT order_mainqt.ddh) as rowcount from b_ry xs,order_mainqt left join order_mxqt on order_mainqt.ddh=order_mxqt.ddh left join order_zh on order_mainqt.ddh=order_zh.ddh and zy<>'订单订金' where order_mainqt.xsbh=xs.bh and (order_mainqt.state='进入生产' or order_mainqt.state='已打印' or order_mainqt.state='生产完成') and not pendtime is null and hdendtime is null and zzfy in $dwdmStr ", $conn);

    $rowcount = mysql_result($restotal,0,'rowcount');

    include '../commonfile/paging_data.php';

    $rs = mysql_query("select order_mainqt.*,count(order_mxqt.id) mxsl,group_concat(order_mxqt.productname) cpms,xs.xm,sksj from b_ry xs,order_mainqt left join order_mxqt on order_mainqt.ddh=order_mxqt.ddh left join order_zh on order_mainqt.ddh=order_zh.ddh and zy<>'订单订金' where order_mainqt.xsbh=xs.bh and (order_mainqt.state='进入生产' or order_mainqt.state='已打印' or order_mainqt.state='生产完成') and not pendtime is null and hdendtime is null and zzfy in $dwdmStr  group by order_mainqt.ddh order by ddate desc limit $startrow ,$pagenum", $conn);
