...
if ($_SESSION["FBSD"] == 1) {//前台
    ...
    if ($_GET["fdd"] <> "") {
        ...
        if($_POST['trouble_order']=='on'){

            $dnow = date('Y-m-d h:',time()).'00:00';
            $troublesql = " and datediff(day,order_mainqt.sdate ,$dnow) > 2 and order_mainqt.state <> '订单完成'";
        ...
        if($_POST['trouble_order']=='on'){
            ...
            $troublesql = " and (to_days(now()) - to_days(order_mainqt.sdate)) > 2 and order_mainqt.state <> '订单完成'";
    ...
    $rs = mysql_query("select order_mainqt.*,count(order_mxqt.id) mxsl,group_concat(order_mxqt.productname) cpms,xs.xm,sksj from b_ry xs,order_mainqt left join order_mxqt on order_mainqt.ddh=order_mxqt.ddh left join order_zh on order_mainqt.ddh=order_zh.ddh and zy<>'订单订金' where order_mainqt.xsbh=xs.bh and (order_mainqt.state='进入生产' or order_mainqt.state='生产完成') and pczy is null and zzfy in $dwdmStr group by order_mainqt.ddh order by ddate desc", $conn);

elseif ($_SESSION["FBHD"] == 1) //生产后加工

    $rs = mysql_query("select order_mainqt.*,count(order_mxqt.id) mxsl,group_concat(order_mxqt.productname) cpms,xs.xm,sksj from b_ry xs,order_mainqt left join order_mxqt on order_mainqt.ddh=order_mxqt.ddh left join order_zh on order_mainqt.ddh=order_zh.ddh and zy<>'订单订金' where order_mainqt.xsbh=xs.bh and (order_mainqt.state='进入生产' or order_mainqt.state='生产完成') and not pczy is null and hdczy is null and zzfy in $dwdmStr  group by order_mainqt.ddh order by ddate desc", $conn);
elseif ($_SESSION["FBFH"] == 1) { //配送
    if ($_POST["_khmc"] <> "")
        $tj = " and order_mainqt.khmc like '%" . $_POST["_khmc"] . "%' ";
    else
        $tj = "";
    $rs = mysql_query("select order_mainqt.*,count(order_mxqt.id) mxsl,group_concat(order_mxqt.productname) cpms,xs.xm,sksj from b_ry xs,order_mainqt left join order_mxqt on order_mainqt.ddh=order_mxqt.ddh left join order_zh on order_mainqt.ddh=order_zh.ddh and zy<>'订单订金' where order_mainqt.xsbh=xs.bh $tj and order_mainqt.state='待配送' and zzfy  in $dwdmStr  group by order_mainqt.ddh order by ddate desc", $conn);
...
<? }
    if ($_SESSION["FBSD"] == 1) { ?>
        ...
        <input onClick="WdatePicker();" maxlength="50" class="Wdate" style="cursor:pointer;" type="text" name="rq2"
               id="rq2" value="<? echo $d2; ?>" size="8" readonly/> 　　<input type="button" value="查找sd" onclick="javascript:window.location.href='MYOrderShowns_trouble.php?fdd='+document.form1.fdd.value+'&filename='+document.form1.filename.value+'&rq1='+document.form1.rq1.value+'&rq2='+document.form1.rq2.value+'&gp='+document.form1.state.options[document.form1.state.selectedIndex].value;"/>
        <!--        <input type="submit" name="bt2" value="导出"/>-->
        <label><input type="checkbox" name="trouble_order" value="on"/>查看问题订单</label>
    ...
    if ($_SESSION["FBCW"] == 1) {
        ...
        <input onClick="WdatePicker();" maxlength="50" class="Wdate" style="cursor:pointer;" type="text" name="rq1"
               id="rq1" value="<? echo $d1; ?>" size="8" readonly/>～
        <input onClick="WdatePicker();" maxlength="50" class="Wdate" style="cursor:pointer;" type="text" name="rq2"
               id="rq2" value="<? echo $d2; ?>" size="8" readonly/> 　　<input type="button" value="查找" onclick="javascript:window.location.href='MYOrderShowns_trouble.php?fdd='+document.form1.fdd.value+'&filename='+document.form1.filename.value+'&rq1='+document.form1.rq1.value+'&rq2='+document.form1.rq2.value+'&gp='+document.form1.state.options[document.form1.state.selectedIndex].value;"/>
    ...
    <? if ($gp == "待结算") { ?>
        ...
        <input value="导出" type="button" onclick="javascript:window.open('../phpExcel/toexcel.php?state=<? echo $gp ?>&fdd=<? echo $_GET['fdd']; ?>&dd1=<? echo $dd1; ?>&dd2=<? echo $dd2; ?>')" name="buttonprint">
