...
if ($_POST["mxids"]<>"") {
    ...
    $id=$_POST["id"];
    if ($id=="") {
        mysql_query("insert into order_mxqt_fm (id,mxid,fmfs,cpcc,jldw,sl,jg,memo,ddh) values (0,'".$_POST["mxids"]."','".$_POST["fmfs"]."','".$_POST["cpcc"]."','".$_POST["jldw"]."','".$_POST["sl"]."','".$_POST["jg"]."','".$_POST["memo"]."','$ddh')",$conn);
    } else {   //有id 明细修改
        mysql_query("update order_mxqt_fm set fmfs='".$_POST["fmfs"]."',cpcc='".$_POST["cpcc"]."',jldw='".$_POST["jldw"]."',sl='".$_POST["sl"]."',jg='".$_POST["jg"]."',memo='".$_POST["memo"]."' where id=$id",$conn);
...
//        echo mysql_result($rshd,0,0)."/";
//        echo mysql_result($rsfm,0,0);
//        exit();
...
if ($_GET["mxhdid"]<>"" ) {
    $id=$_GET["mxhdid"];
    $rs=mysql_query("select * from order_mxqt_fm where id=".$_GET["mxhdid"]);
    $fmfs=mysql_result($rs,0,"fmfs");
...
//          有数量的价格不能为零
        ...
        function set()
        {
            ...
            $.ajax({
                type: "get",
                url: "getprice.php?type=4&ddh="+$("#ddh").val()+"&jgfs="+$("#jgfs").val()+"&cpcc="+$("#cpcc").val()+"&jldw="+$("#jldw").val(),
                async: false,
        ...
        <tr>
            <td height="222" valign="top">
                <table width="95%" height="153" border="0" align="center">
                    ...
                    <tr>
                        <td width="78" height="27" class="STYLE13">覆膜方式</td>
                        <td colspan="2" class="STYLE13">
                            <select name="fmfs" id="jgfs" onChange="window.location.href='YSXMqt_mxdjs_fm.php?ddh=<? echo $ddh?>&mxids=<? echo $_GET["mxids"]?>&pn=<? echo $pn?>&fmfs='+this.options[this.selectedIndex].value;">
                                <option value="-1">请选择...</option>
                                <? $rs1=mysql_query("select distinct fumo from b_fumo order by fumo");
                                if($rs1)
                                    for ($i=0;$i<mysql_num_rows($rs1);$i++) {?>
                                        <option value="<? echo mysql_result($rs1,$i,0);?>" <? if ($fmfs==mysql_result($rs1,$i,0)) echo "selected";?> ><? echo mysql_result($rs1,$i,0);?></option>
                                    <? }?>
                            </select>
                        </td>
                    </tr>
                                ...
                                <table width="100%" border="1" align="center" cellspacing="0" cellpadding="0" style="border-width:0px;border-collapse:collapse;border:1px solid #D3D3D3; border-bottom:1px solid #D3D3D3; font-size:12px">
                                    ...
                                    <tr>
                                        ...
                                        <td width="30%"><select name="cpcc" id="cpcc" onchange="set()" >
                                                <? $rs1=mysql_query("select distinct chicun from b_fumo where fumo ='$fmfs' order by chicun");
                                                if($rs1)
                                                    for ($i=0;$i<mysql_num_rows($rs1);$i++) {?>
                                        ...
                                        <td width="34%"><select name="jldw" id="jldw" >
                                                <? $rs1=mysql_query("select distinct unit from b_fumo where fumo='$fmfs' order by unit");
                                                if($rs1)
                                                    for ($i=0;$i<mysql_num_rows($rs1);$i++) {?>
