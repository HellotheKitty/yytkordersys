...
//    files  save
    $filedir = 'server/upload/' . $ddh . '/' . $mxid .'/';
    $file = scandir($filedir);
    $file1 = '';
    if($file[2]){
        $file[2] = iconv('gbk' , 'utf-8' , $file[2]);
        if($_SESSION['GDWDM'] == '330100'){
            $file1 = 'http://59.110.17.13/ordersys/customer/neworder/' . $filedir . $file[2];

        }else{
            $file1 = 'http://oa.skyprint.cn/customer/neworder/' . $filedir . $file[2];

        }
    }


    $sql = "update order_mxqt set dsm1='$dsm1' ,hzx1 = '$hzx1' ,
 sl1 = $sl1 ,jg1 = $jg1 ,paper1 = $materialid , machine1= '$machine1',
 pnum1=$pnum1 , pname = '$pname' , sczzbh1='$sczzbh1' , file1 = '$file1'
 where id=$mxid";
...
if($_GET['addfm']<>''){
    ...
    if($_GET['fmid']<>''){
        ...
        $fmid = $_GET['fmid'];
        $sqladdfm[] = mysql_query("update order_mxqt_fm set fmfs='".$_GET["fmfs"]."',cpcc='".$_GET["chicun"]."',jldw='P',sl='". intval($_GET["fmsl"]) ."',jg='". floatval($_GET["fmjg"]) ."',memo='".$_GET["fmmemo"]."' where id=$fmid",$conn);

    }else{
        $sqladdfm[] = mysql_query("insert into order_mxqt_fm (id,mxid,fmfs,cpcc,jldw,sl,jg,memo,ddh) values (0,'".$_GET["mxid"]."','".$_GET["fmfs"]."','".$_GET["chicun"]."','P','".intval($_GET["fmsl"])."','" . floatval($_GET["fmjg"]) ."','".$_GET["fmmemo"]."','$ddh')",$conn);
...
//    发货信息保存在fh_info表里面
    ...
    header("location:newpiece.php?ddh=".$_POST["ddh"]."&mxid=".$mxid);
    exit;
}
...
if ($_GET["deleid"]<>"") {
    ...
    header("location:newpiece.php?ddh=".$ddh."&auth=".md5($_SESSION["KHID"]."-".$_SESSION["AUTHCODE"]."-".$ddh."-"."新建订单"));
    exit;
}
...
//        tbsj : 单张下单
    $memo = '客户单张下单';
        mysql_query("insert into order_mainqt (ddh,khmc,xsbh,ddate,state,sdate,shr,shdh,shdz,memo,scjd,xjdid,zzfy,filefy,tbsj) values ('".$bh."','".$khmc."','".$xsbh."',now(),'新建订单',now(),'$lxr','$lxdh','$lxdz','$memo','".$_SESSION["GSSDQ"]."','','$dwdm','".$_SESSION["INFO"]["loginname"]."' , 'dzxd')",$conn);

    if($_GET['copyddh']<>''){

        $copyddh = $_GET['copyddh'];
        $newddh = $bh;
        include '../../commonfile/copy_order.php';
        echo "<script>window.location.href='newpiece.php?ddh=$bh'</script>";
        exit;
    }
//增加一条default明细
        include('new_mx.php');
...
//        $updje = mysql_query("update order_mainqt set dje = '$je' WHERE ddh = $bh",$conn);
        ...
        header("location:newpiece.php?ddh=".$bh."&mxid=".$mxid);
        exit();
                    ...
                    <tr>
                        ...
                        <td colspan="3" class="STYLE13">
                            <select name="scjd">
                                ...
                                <option value="北京" <? if(mysql_result($rs,0,"scjd")=='北京') echo 'selected'; ?>>北京</option></select>
                            ...
                            <input type="submit" name="button" id="button" value="保存订单信息">
                        </td>
                    </tr>
                    <tr>
...
<form name="formmx" action="function/ordermxsave.php?ddh=<?echo $bh?>" method="get" enctype="multipart/form-data">

<div class="pinban_view">
    <div id="demo" class="fileupbox">
        <div id="test"></div>
        <br/>
        <? if(is_dir('server/upload/' . $_GET['ddh'] . '/' . $_GET['mxid'] . '/')) echo '<span style="color:#555;">已上传文件</span>'; ?>
...
<!--            <a id="uploadfiles" href="javascript:;">[上传文件]</a>-->
            ...
            <form id="form1">
                ...
                <input type="text" id="userid" value="<? echo $bh . '/' . $_GET['mxid']; ?>" style="display:none;">
                <input type="text" id="blocklen" value="1000000" style="display:none;">
                <input type="hidden" id="piece" value="1"/>
            </form>
            ...
            <span style="color:#FF0000">注意：文件名不能含有空格！</span>
            <!--                end of uploadex-->
            <br>
        </div>
    ...
    <!--<div class="fileupbox">
        <input type="button" value="上传文件 ->" datatype="<?/* echo$_GET['mxid'] */?>" id="reupfile" class="uploadpdfbtn"/>
        <p style="font-size:10px;color:#555;">请上传已经转曲的pdf文件</p>
    </div>-->
...
//        $rsmx = mysql_query("select order_mxqt.*,m1.materialname mm1,m1.specs ms1,m2.materialname mm2,m2.specs ms2,group_concat(hd.jgfs) hd,sum(hd.jg*hd.sl) hdje,group_concat(hd.jg) hdjg, group_concat(fm.fmfs) fmfs,sum(fm.jg*fm.sl) fmje,group_concat(fm.jg) fmjg from order_mxqt left join order_mxqt_hd hd on order_mxqt.id=hd.mxid left join order_mxqt_fm fm on order_mxqt.id=fm.mxid LEFT JOIN material m1 on  m1.id=paper1 LEFT JOIN material m2 on m2.id=paper2 where order_mxqt.ddh='" . $_GET['ddh'] . "'  group by order_mxqt.id", $conn);
        ...
        $pname = mysql_result($rsmx,$y,'pname'); //印件名 自带纸
        $sczzbh1 = mysql_result($rsmx,$y,'sczzbh1');
...
//                    if($machine1 == 'Hp7600彩色') {
//
//                        if ($item['Specs'] == '750*530') {
//                            continue;
//                        }
//                    }
//                    if($machine1 == 'Hp10000彩色'){
//
//                        if($item['Specs'] == '464*320') continue;
//                    }
                    ?>
            ...
            <span id="pnameoutter" style="display: none;">
                ...
                <select id="selfpaper_zzbh" name="selfpaper_zzbh">
                    <option value=""></option>
                    ...
                    if(mysql_num_rows($respname)>0){
                        ...
                        while($pnameitem = mysql_fetch_array($respname)){
                            ...
                            ?>
                            <option value="<? echo $pnameitem['scbh'] ?>" <? if($sczzbh1 == $pnameitem['scbh']){echo ' selected';} ?>><? echo $pnameitem['zzmc'] ?></option>
                            <?
                        }
                    }
                    ?>
            ...
            <span>横纵向</span>
<!--            <input type="text" id="hzx" name="hzx" value="--><?// echo $hzx1; ?><!--" />-->
            <select id="hzx" name="hzx" >
                <option value="">请选择横纵向</option>
            ...
            <span>印件名</span>
            <input type="text" id="pname" name="pname" value="<? echo $pname; ?>"/>


            &nbsp;&nbsp;
            <input type="button" name="" id="savemx" value="保存明细" class="webuploader-pick"/>
                        ...
                        <td><select name="hdfs" id="hdfs" class="setjg">
                                ...
                                if($rs1){
                                    for ($i=0;$i<mysql_num_rows($rs1);$i++) {?>
                                        <option value="<? echo mysql_result($rs1,$i,0);?>" ><? echo mysql_result($rs1,$i,0),'-',mysql_result($rs1,$i,'unit');?></option>
                                    <? }
                                } ?>
                            </select>
                        </td>
...
$('#savemx').on('click',function(){
    ...
    var _mxid = $('#mxid').val();
    var _selfpaper_zzbh = $('#selfpaper_zzbh').val();
//    var _pname = $('#selfpaper_zzbh').find('option:selected').html();
    var _pname = $('#pname').val();

    var _senddata = "savemx=" + _mxid + "&machine=" + _machine + "&material=" + _material + "&dsm=" + _dsm + "&pnum=" + _pnum + "&sl="+ _sl + "&hzx=" + _hzx + "&jg1=" + _jg + "&selfpaper_zzbh=" + _selfpaper_zzbh + "&pname=" + _pname ;
...
$('#selfpaper_zzbh').on('change',function(){

    $('#pname').val($('#selfpaper_zzbh').find('option:selected').html());
});
...
/*
    ...
    var uploader = new plupload.Uploader({
        ...
        url : 'server/upload.php?bh=<? //echo $bh;?>&mxid='+formmx.mxid.value,
    ...
    function fileSelect1(e)
    {
        ...
        for(var i = 0, len = files.length; i < len; i++) 	{
            ...
            html.push(
                ...
                '<span class="upload_percent" style="display:none;">0%</span> <br/><span class="totalsize">文件大小：' + f.size + 'bytes</span>'+'<span class="up_filename">'+f.name + '</span><span class="delfilebtn" fid="'+fileID+'">X</span>',
                '</div>'
            );
        }
...
//    upload
//    $(document).on('click','#reupfile',function(){
//
//        var _mxid = $(this).attr('datatype');
//        window.open('../order/upload/uploadpdf.php?mxid='+_mxid,'Orderjs','height=400px,width=720px,top=100px,left=200px,location=no,menubar=no,resizable=no,scrollbars=yes,status=no,titlebar=no,toolbar=no,directories=no')
//
//    });
