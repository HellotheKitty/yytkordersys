...
if($_GET['savemx']<>''){
    ...
    $jg1 = $_GET["jg"] ? $_GET["jg"] : 0; //price

    $materialid2 = $_GET['material2'] ?  $_GET['material2'] : 1;       //纸张种类
    $dsm2 = $_GET['dsm2'];                 //单双面
    $machine2 = $_GET["machine2"];            //机器
    $pnum2 =$_GET["pnum2"] ;             //页数
    $sl2 = $_GET["sl2"]?$_GET["sl2"]:0;       //数量
    $jg2 = $_GET["jg2"] ? $_GET["jg2"] : 0; //price


//生产纸张编号
    $sczzbh1 = $_GET['selfpaper_zzbh'];
    $sczzbh2 = $_GET['selfpaper_zzbh2'];
...
//生产纸张编号2
    if($sczzbh2 ==''){
        ...
        $res_zzbh = mysql_query("select memo from material where MaterialCode = $materialid2",$conn);

        if(mysql_num_rows($res_zzbh)>0){

            $sczzbh2 = mysql_result($res_zzbh , 0 ,'memo');
        }else{
            $logfile = new Log();
            $logfile ->NOTICE($materialid2 .'没有承印物');
        }
    }

    $sql = "update order_mxqt set dsm1='$dsm1' , sl1 = $sl1 ,paper1 = $materialid , machine1= '$machine1',pnum1=$pnum1 , pname = '$pname' , sczzbh1='$sczzbh1', jg1=$jg1 ,
 dsm2='$dsm2' , sl2 = $sl2 ,paper2 = $materialid2 , machine2= '$machine2',pnum2=$pnum2 , sczzbh2='$sczzbh2' , jg2 = $jg2 where id=$mxid";
...
//    $skfs=$_POST["butt2"];
    ...
    mysql_query("update order_mainqt set khmc='$khmc',psfs='".$_POST["psfs"]."',kdje='".$_POST["kdje"]."',shr='".$shr."',shdh='".$shdh."',shdz='".$shdz."',scjd='$scjd',bzyq='$bzyq',kpyq='$kpyq',memo='$memo',yqwctime='".$_POST["yqwctime"]."',djje='".$_POST["djje"]."',hdczy='$collate' where ddh='".$_POST["ddh"]."'",$conn);
...
//    发货信息保存在fh_info表里面
    ...
    header("location:newbook.php?ddh=".$_POST["ddh"]."&mxid=".$mxid);
    exit;
}
...
if ($_GET["deleid"]<>"") {
    ...
    header("location:newbook.php?ddh=".$ddh."&auth=".md5($_SESSION["KHID"]."-".$_SESSION["AUTHCODE"]."-".$ddh."-"."新建订单"));
    exit;
}
...
//        tbsj : 单张下单
    $memo = '客户书本拼版';
    mysql_query("insert into order_mainqt (ddh,khmc,xsbh,ddate,state,sdate,shr,shdh,shdz,memo,scjd,xjdid,zzfy,filefy,tbsj) values ('".$bh."','".$khmc."','".$xsbh."',now(),'新建订单',now(),'$lxr','$lxdh','$lxdz','$memo','".$_SESSION["GSSDQ"]."','','$dwdm','".$_SESSION["INFO"]["loginname"]."' , 'sbpb')",$conn);

//增加一条default明细
    include('new_mx_book.php');
...
//        $updje = mysql_query("update order_mainqt set dje = '$je' WHERE ddh = $bh",$conn);
    ...
    header("location:newbook.php?ddh=".$bh."&mxid=".$mxid);
    exit();
...
<style type="text/css">
    ...
    #pnum,#sl,#jg,#hdjg,#fmjg,#hdsl,#fmsl,#pnum2,#sl2,#jg2,#hdjg2,#fmjg2,#hdsl2,#fmsl2{width:40px;}
    #jg,#hdjg,#fmjg,#pnum,#jg2,#hdjg2,#fmjg2,#pnum2{color:#888}
    .ordermxtb {
                ...
                <input name="yqwctime" type="text" value="<? echo mysql_result($rs,0,"yqwctime")==""?date("Y-m-d H:i:s",strtotime("+1 day",strtotime(mysql_result($rs,0,"ddate")))):mysql_result($rs,0,"yqwctime")?>" size="15"></span>
                    ...
                    <tr>
                        ...
                        <td class="STYLE13"><font color="red">账户余额：</font><?
                            ...
                            $res_ye = mysql_query("SELECT sum(ye + czxf) as ye from kh_ye LEFT JOIN (SELECT sum((ifnull(`order_zh`.`jf`, 0) - ifnull(`order_zh`.`df`, 0))) as czxf ,khmc FROM order_zh WHERE fssj > IFNULL((SELECT sdate FROM kh_ye LIMIT 1),'2015-01-01') and khmc='$khmc' ) a on kh_ye.depart = a.khmc LEFT JOIN base_kh on kh_ye.depart = base_kh.khmc where base_kh.khmc = '$khmc' ",$conn);
                            if(mysql_num_rows($res_ye)>0){
                                $ye = mysql_result($res_ye,0,'ye');
                                echo $ye.'元';
                            }
                            ?></td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td colspan="3" class="STYLE13"><!--<input name="scjd" id="scjd" type="text" value="<? echo mysql_result($rs,0,"scjd");?>" size="12" maxlength="4">-->
                            ...
                            <label>
                                <input type="checkbox" name="collate" <? if(mysql_result($rs,0,"hdczy")==1) echo 'checked'; ?> value="1"/>
                                按序配页
                            </label>
            ...
            <td>
                <table cellspacing="0" cellpadding="0" rules="all" border="1" id="gvOrder" class="ordermxtb">
                    ...
                    <tr style="height:30px;">
                        ...
                        <th width="93" scope="col">生产文件(封面)</th>
                        <th width="120" scope="col">封面信息</th>
                        <th width="35"  scope="col">封面数量</th>

                        <th width="93" scope="col">生产文件(内页)</th>
                        <th width="120" scope="col">内页信息</th>
                        <th width="35"  scope="col">内页数量</th>
                    ...
                    for($i=0;$i<mysql_num_rows($rsmx);$i++){  ?>
                        <tr style="height:30px;">
                            ...
                            <td>
                                <? $machine2 = mysql_result($rsmx,$i,"machine2");
                                $material2 =mysql_result($rsmx,$i,"mm2");
                                $dsm2 = mysql_result($rsmx,$i,"dsm2");
                                $hzx2 = mysql_result($rsmx,$i,"hzx2");
                                $pnum2 = mysql_result($rsmx,$i,"pnum2");
                                $sl2 = mysql_result($rsmx,$i,"sl2");
                                $jg2 = mysql_result($rsmx,$i,'jg2');
                                $chicun2 = mysql_result($rsmx,$i,"ms2");
                                $paper2 = mysql_result($rsmx,$i,'paper2');
                                $pname = mysql_result($rsmx,$i,'pname');

                                echo $machine2,
                                "/<font color=red>",$material2,
                                "[",$chicun2,"]</font>/",
                                mysql_result($rsmx,$i,"jldw2"),"/",
                                $dsm2,"/",
                                $hzx2,"/P:",
                                $pnum2,"/SL:",
                                $sl2
                            ...
                            <td><? echo mysql_result($rsmx,$i,"sl2");?></td>
...
<!---------订单信息拼版信息分界线---------->
<form action="function/pinban_kh_book.php?ddh=<? echo $bh ?>" method="post" enctype="multipart/form-data">

    <div class="mx-outter options" id="options" datatype="printopt">
        <table class="mxopt">
            <tr>
                <td>
                    <div class="options mxeditbox" id="mxeditbox" datatype="printopt">
                        <span>打印方式:</span><br>
                        <input type="hidden" value="<? echo $_GET['ddh']; ?>" name="ddh" id="ddh"/>
                        <input type="hidden" value="<? echo $mxid; ?>" name="mxid" id="mxid"/>
                        <input type="hidden" value="<? echo $chicun1; ?>" name="chicun" id="chicun"/>

                         <div id="pcover">
                            封面:
                             <select id="machine" name="machine" class="selmm setjg machine">
                            ...
                            <select id="material" name="material" class="selmm setjg material">
                        ...
                        <span class="pnameoutter" style="display: none;">
                            ...
                            <select id="dsm" name="dsm" class="setjg dsm">
                            ...
                            <span>P数</span>
                            <input type="text" id="pnum" name="pnum" class="pnum" value="<? echo $pnum1; ?>" />
                            <span>份数<span style="color:red;display:inline;">*</span></span>
                            <input type="text" id="sl" class="setjgblur sl" name="sl" value="<? echo $sl1; ?>"/>

                            <span>单价</span>
                            <input type="text" id="jg" class="jg" readonly="readonly" name="jg" value="<? echo $jg1; ?>"/>

                        </div>

                        <div id="pinner">
                            内页:
                            <select id="machine2" name="machine2" class="selmm setjg machine">
                                <option value="">请选择机型</option>
                                <option class="hp10000machine" value="Hp10000彩色" <? if($machine2=='Hp10000彩色') echo 'selected';  ?>>Hp10000</option>
                                <option class="hp7600machine" value="Hp彩色" <? if($machine2=='Hp彩色') echo 'selected';  ?>>Hp7600</option>
                            </select><span style="color:red;display:inline;">*</span>
                            <select id="material2" name="material2" class="selmm setjg material">
                                <option value="">请选择纸张</option>
                                <? $resmaterial = mysql_query("select MaterialName,MaterialCode, Specs from material where zzfy = $dwdm and locate('A3',MaterialName)=0 and locate('A4',MaterialName) = 0" ,$conn);

                                while($item = mysql_fetch_array($resmaterial)){
                                    ?>
                                    <option class="<? if($item['Specs'] == '750*530') echo 'hp10000'; elseif($item['Specs']=='464*320') echo 'hp7600'; ?>" value="<? echo $item['MaterialCode'];?>" <?  if($paper2 ==$item['MaterialCode']) echo 'selected'; ?>><? echo $item['MaterialName'] ?></option>
                                    <?
                                }
                                ?>
                            </select><span style="color:red;display:inline;">*</span>
                        ...
                        <span class="pnameoutter" style="display: none;">
                            <span>具体纸张名:<span style="color:red;display:inline;">*</span></span>
                            <select id="" name="selfpaper_zzbh2">
                                <option value="">请选择...</option>
                                <? $respname = mysql_query("select zzmc,scbh from base_zz_ck order by convert(zzmc using gbk)",$connykgf);
                                if(mysql_num_rows($respname)>0){
                            ...
                            <select id="dsm2" name="dsm2" class="setjg dsm">
                                <option value="">请选择单双面</option>
                            ...
                            <span>P数</span>
                            <input type="text" id="pnum2" name="pnum2" class="pnum" value="<? echo $pnum2; ?>" />
                            <span>份数<span style="color:red;display:inline;">*</span></span>
                            <input type="text" id="sl2" class="setjgblur sl" name="sl2" value="<? echo $sl2; ?>"/>

                            <span>单价</span>
                            <input type="text" id="jg2" readonly="readonly" class="jg" name="jg2" value="<? echo $jg2; ?>"/>
...
<!--                <a id="uploadfiles" href="javascript:;">[上传文件]</a>-->
                ...
                <span style="margin:10px 0;display: inline-block;">
                    请选择拼版方式:
                    <select name="pb_type1" style="width:100px;height:25px;">
                        <option value="staple">骑马钉拼版</option>
                        <option value="binding">胶装拼版(骑马钉拼法)</option>
                        <option value="bind_normal">胶装拼版(单张拼法)</option>
                    </select>
                </span>
                <br>

                <input type="submit" name="" id="" value="点击生成 ->" class="webuploader-pick"/>
                <br><span>格式:pdf</span><br>
                <span>要求:1、pdf按页码顺序排列（包含封面封底，单面请插入空白页）;页数必须是<span style="color:red;font-weight:bold;">4的倍数。</span></span><br>
                <span>2、胶装拼版封面请<span style="color:red;font-weight:bold;">做好书脊。</span><a href="tools/calc_bookbone.php" target="_blanks">书脊宽度计算</a></span>
            </div>
            <br />
            <pre id="console"></pre>
        ...
        <div id="show"  class="preview clearfix">
            <p>
                拼版预览
                <input type="button" value="删除预览" id = 'del_preview'/>
            ...
            <div id="img_outer">
                ...
                if(is_dir($path)){
                    ...
                    foreach($filearr as $image_url){
                        ?>
                        <img src="<?echo $path.$image_url?>" style="<? if(explode('-' , explode('.',$image_url)[0])[1] == '2') echo 'float:right'; else echo 'float:left'; ?>" class="preimg"/>
                        <?
                    }
                }else{
                    ?>
                    <img class="preimg" src="function/BillFiles/previewcover.png" />
                    <img class="preimg" src="function/BillFiles/previewinner.png" />
...
/*
    ...
    var uploader = new plupload.Uploader({
        ...
        url : 'server/upload_pb.php?bh=<?echo $bh;?>&mxid=<? echo $mxid; ?>',
...
//显示相应尺寸的纸张
    $('.selmm').on('change',function(){
        ...
        if(_cur == 'material' || _cur == 'material2'){


        }else if(_cur == 'machine' || _cur == 'machine2'){
...
//                订单金额更新
    ...
    function setjg(_this){
        ...
        if(ptype == 'printopt'){
            ...
            var goujian = _this.closest('div');

            _senddata +='&type=' + type +'&machine=' + goujian.find('.machine').val() +'&paper=' + goujian.find('.material').val()
                + '&dsm=' + goujian.find('.dsm').val() +'&zsl='+ (parseInt(goujian.find('.pnum').val()) * parseInt(goujian.find('.sl').val())) + '&jldw=p';
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                if(type=='1'){_this.closest('div').find('.jg').val(data);}
    ...
    //    自带纸选择印件名
    $('#mxeditbox').find('.material').on('change',function(){

        var _text = $(this).find('option:selected').html();

        if(_text.indexOf('自带') >=0){

            $(this).closest('div').find('.pnameoutter').show();

        }else{
            $(this).closest('div').find('.pnameoutter').hide();
        }
    });
    ...
    $('#savemx').on('click',function(){
        ...
        var _machine2 = $('#machine2').val();
        var _material2 = $('#material2').val();
        var _dsm2 = $('#dsm2').val();
        var _pnum2 = $('#pnum2').val();
        var _sl2 = $('#sl2').val();
        var _jg2 = Number($('#jg2').val());

        var _senddata = "savemx=" + _mxid + "&machine=" + _machine + "&material=" + _material + "&dsm=" + _dsm + "&pnum=" + _pnum + "&sl="+ _sl + "&jg=" + _jg;

        _senddata += "&machine2=" + _machine2 + "&material2=" + _material2 + "&dsm2=" + _dsm2 + "&pnum2=" + _pnum2 + "&sl2="+ _sl2 + "&jg2=" + _jg2;
...
//    删除预览
    $('#del_preview').on('click',function(){
        ...
        $.ajax({
            ...
            type : "GET",
            dataType : 'json',
            url : 'function/function/delete_preview.php?ddh='+ ddh,
            success: function(){

                var _html = '<img class="preimg" src="function/BillFiles/previewcover.png">' +
                    '<img class="preimg" src="function/BillFiles/previewinner.png">';
                $('#img_outer').html(_html);
            },
            error: function(){
                alert('err,plz retry');
            }
