    ...
    public function __construct($dbName = 'ordersys')
    {
        try {
            $DB = [
                ...
                'HOST' => 'rdsfifeue7feyufi.mysql.rds.aliyuncs.com:3386',

                'USER' => 'yikamuser',

                'PASSWORD' => 'Yikayin88382383',

                'CHARSET' => 'UTF8'


            ];
            $this->dsn = 'mysql:host='.$DB['HOST'].';dbname='.$dbName;
            $this->dbh = new PDO($this->dsn, $DB['USER'], $DB['PASSWORD']);
            $this->dbh->exec('SET character_set_connection='.$DB['CHARSET'].', character_set_results='.$DB['CHARSET'].', character_set_client=binary');
