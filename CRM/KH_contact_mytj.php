...
exit; }?>
<? 
 $rs=mysql_query("select bh,xm,team from crm_callconfig where bh='".$_SESSION["YKOAUSER"]."' and qx='100' ",$conn);
?> 
...
<DIV ID=MainArea>
            ...
            <? for($i=0;$i<mysql_num_rows($rs);$i++){  ?>
					<tr onMouseOver="this.style.backgroundColor='#FFD'" onMouseOut="this.style.backgroundColor='white'"  class="TableDetail2" id="d0">
                            ...
                            <? $rstt=mysql_query("select count(1),group_concat(khmc,'\n') from crm_khb where left(xsry,instr(xsry,'/')-1)='".mysql_result($rs,$i,"bh")."' and (state=-2 or state=1) and xsryfpsj>'".date('Y-m-d')."'",$conn);?>
							<TD align="center" title="<? echo mysql_result($rstt,0,1);?>"><font color="red"><? echo mysql_result($rstt,0,0);?></font></TD>
                            ...
                            <? $rstt=mysql_query("select count(1),GROUP_CONCAT( distinct khmc,'\n') from (select count(distinct crm_khb.id),khmc from crm_khb,nc_erp.order_main where left(xsry,instr(xsry,'/')-1)='".mysql_result($rs,$i,"bh")."' and crm_khb.state>0 and yikayin_zh=order_main.user and order_main.dje>0 group by crm_khb.id having min(ddate)>'".date('Y-m-d')."') cc",$conn);?>
							<TD align="center" title="<? echo mysql_result($rstt,0,1);?>"><? echo mysql_result($rstt,0,0);?></TD>
							...
							<TD align="center" bgcolor="#99FFFF" ><? echo mysql_result(mysql_query("select count(1) from crm_khb_contact c,crm_khb where c.khbid=crm_khb.id and czy='".mysql_result($rs,$i,"bh")."' and (state=-1 or (state=-2 and xsryfpsj>'".this_monday(0,false)."')) and czsj>'".this_monday(0,false)."' and content<>'占线或无人接听'",$conn),0,0);?></TD>
							 <? $rstt=mysql_query("select count(1),group_concat(khmc,'\n') from crm_khb where left(xsry,instr(xsry,'/')-1)='".mysql_result($rs,$i,"bh")."' and (state=-2 or state=1) and xsryfpsj>'".this_monday(0,false)."'",$conn);?>
                            <TD align="center" bgcolor="#99FFFF" title="<? echo mysql_result($rstt,0,1);?>"><font color="red"><? echo mysql_result($rstt,0,0);?></font></TD>
							<TD align="center" bgcolor="#99FFFF" ><? echo mysql_result(mysql_query("select count(1) from crm_khb_contact c,crm_khb where c.khbid=crm_khb.id and czy='".mysql_result($rs,$i,"bh")."' and (state=-2 and xsryfpsj<='".date('Y-m-d')."') and czsj>'".this_monday(0,false)."'",$conn),0,0);?></TD>
                             <? $rstt=mysql_query("select count(1),GROUP_CONCAT( distinct khmc,'\n') from (select count(distinct crm_khb.id),khmc from crm_khb,nc_erp.order_main where left(xsry,instr(xsry,'/')-1)='".mysql_result($rs,$i,"bh")."' and crm_khb.state>0 and yikayin_zh=order_main.user and order_main.dje>0 group by crm_khb.id having min(ddate)>'".this_monday(0,false)."') cc",$conn);?>
							<TD align="center" title="<? echo mysql_result($rstt,0,1);?>"><? echo mysql_result($rstt,0,0);?></TD>
                            ...
                            <? $rstt=mysql_query("select count(1),group_concat(khmc,'\n') from crm_khb where left(xsry,instr(xsry,'/')-1)='".mysql_result($rs,$i,"bh")."' and (state=-2 or state=1) and xsryfpsj>'".date('Y-m-')."01'",$conn);?>
							<TD align="center" title="<? echo mysql_result($rstt,0,1);?>"><font color="red"><? echo mysql_result($rstt,0,0);?></font></TD>
							<TD align="center" ><? echo mysql_result(mysql_query("select count(1) from crm_khb_contact c,crm_khb where c.khbid=crm_khb.id and czy='".mysql_result($rs,$i,"bh")."' and (state=-2 and xsryfpsj<='".date('Y-m-d')."') and czsj>'".date('Y-m-')."01'",$conn),0,0);?></TD>
                             <? $rstt=mysql_query("select count(1),GROUP_CONCAT( distinct khmc,'\n') from (select count(distinct crm_khb.id),khmc from crm_khb,nc_erp.order_main where left(xsry,instr(xsry,'/')-1)='".mysql_result($rs,$i,"bh")."' and crm_khb.state>0 and yikayin_zh=order_main.user and order_main.dje>0 group by crm_khb.id having min(ddate)>'".date('Y-m-')."01') cc",$conn);?>
							<TD align="center" title="<? echo mysql_result($rstt,0,1);?>"><? echo mysql_result($rstt,0,0);?></TD>
							...
							<TD align="center" bgcolor="#99FFFF" ><? echo mysql_result(mysql_query("select count(distinct crm_khb.id) from crm_khb,nc_erp.order_main where left(xsry,instr(xsry,'/')-1)='".mysql_result($rs,$i,"bh")."' and crm_khb.state>0 and yikayin_zh=order_main.user and order_main.dje>0",$conn),0,0);?></TD>
