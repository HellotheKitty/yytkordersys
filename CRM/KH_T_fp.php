...
$page_num=15;    
$total = mysql_fetch_array(mysql_query("select count(1) from crm_khb where callout is null and xsry is null and not datainputsj is null and instr((select dq from crm_teamdq where team='".$_SESSION["CRM_TEAM"]."'),province)>0"));//查询数据库中一共有多少条数据 
$Total = $total[0];                       //    
...
$rs=mysql_query("select crm_khb.id,khmc,lxr,sshy,province,city,khcsd,memo,datainputsj,count(crm_khb_contact.id) lxcs from crm_khb left join crm_khb_contact on crm_khb.id=crm_khb_contact.khbid where callout is null and xsry is null and not datainputsj is null and instr((select dq from crm_teamdq where team='".$_SESSION["CRM_TEAM"]."'),province)>0 group by crm_khb.id order by crm_khb.id limit $startnum,$page_num", $conn); 	
...
?>
<hr>
把：<input name="sjsl" type="text" value="<? echo $Total;?>" style="width:50px">条数据  
