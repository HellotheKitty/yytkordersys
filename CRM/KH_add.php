...
if ($_GET["team"]<>"") {
	...
	$rscf=mysql_query("select fjh,http,filterstr,dq from crm_callconfig c,crm_teamdq d where c.team=d.team and bh='".$_SESSION["YKOAUSER"]."' and c.team='".$_GET["team"]."' and qx>'000'",$conn);
	$_SESSION["CRM_FJH"]=mysql_result($rscf,0,"fjh");
	$_SESSION["CRM_HTTP"]=mysql_result($rscf,0,"http");
	$_SESSION["CRM_DQ"]=mysql_result($rscf,0,"dq");
	...
	if ($_GET["id"]<>"") {
		if (substr($_GET["id"],0,4)=="next") {
			if ($_GET["id"]=="next") { //电呼取数
				if (substr($_SESSION["CRM_HTTP"],0,3)=="yun") //云呼叫去掉总机
					$rs=mysql_query("select * from crm_khb where (calloutfpsj is null or timestampdiff(minute,calloutfpsj,now())>=20) and (nextlx is null or datediff(now(),nextlx)>=0) and (state is null or state=-1) ".$_SESSION["CRM_FSTR"]." and xsry is null and (lxdh <>'' or lxrmobile<>'') and instr('".$_SESSION["CRM_DQ"]."',province)>0 and (fzjg<>'总机' or fzjg is null) and POSITION('-' IN substr(crm_khb.lxdh,6))=0 and POSITION('/' IN substr(crm_khb.lxdh,6))=0 order by id limit 1",$conn);
				else
					$rs=mysql_query("select * from crm_khb where (calloutfpsj is null or timestampdiff(minute,calloutfpsj,now())>=20) and (nextlx is null or datediff(now(),nextlx)>=0) and (state is null or state=-1) ".$_SESSION["CRM_FSTR"]." and xsry is null and (lxdh <>'' or lxrmobile<>'') and instr('".$_SESSION["CRM_DQ"]."',province)>0 order by id limit 1",$conn);
				...
				if (mysql_num_rows($rs)>0) {
					...
					mysql_query("insert into crm_czlog values (0,'".$_SESSION["YKOAUSER"]."',now(),'".mysql_result($rs,0,"khmc")."')",$conn);
					if(mysql_errno()){
		    			mysql_query('rollback') or die(mysql_error());
						$rs=mysql_query("select * from crm_khb where 1=2",$conn);
					}else{
...
<input type="hidden" name="id" value="<? echo $id;?>" />
    ...
    <div onClick="if (actForm.khmc.value!='') {$.post('KH_saveinfo.php', $('#actForm').serialize(),function(data){alert('Data Loaded: ' + data);});} else alert('客户名称请输入！');" class="Btn">
        保 存
    </div>
		...
		<DIV ID=MainArea>
            ...
            <a href='void(0)' onclick='javascript:actForm.lxrzw.value="公司经理";return false;'>公司经理</a>           
						...
						<TR>
						    ...
						    <? if (substr($_SESSION["CRM_TEAM"],0,1)!="D" and ($lxrmobile<>'' or $lxdh<>'')) {?><input id="Button1" type="button" value="联系/呼叫" onClick="ShowDiv('MyDiv','fade')" /><? ;if ($nextlx>date("Y-m-d")) echo "<font color=red>下次联系:",$nextlx,"</font>";}?></TD>
						</TR>
						<TR>
							<TD HEIGHT=27></TD>
							<TD>电话号</TD>
            ...
            <a href='void(0)' onclick='javascript:actForm.khygs.value="200人以上";return false;'>200以上</a>           
                        ...
                        <? }
						if ($_SESSION["CRM_QX"]=="000" or $_GET["from"]=="service") {?>
						<TR>
							<TD width="50"></TD>
							<TD width="80">所属销售</TD>
  ...
  <DIV ID=InputDetailBar>
						<TABLE BORDER=0 CELLSPACING=0 CELLPADDING=10 ALIGN=center>
							<TR>
								<TD> 
<div onClick="if (actForm.khmc.value!='') {$.post('KH_saveinfo.php', $('#actForm').serialize(),function(data){alert('Data Loaded: ' + data);});} else alert('客户名称请输入！');" class="FuncBtn"><div class=FuncBtnHead></div>
...
<span style="font-size: 16px;" onClick="CloseDiv('MyDiv','fade')">隐藏</span>
</div>
...
联系情况：<select name="khcsd1" onChange="khcsdc(this);">
...
<? if (substr($_SESSION["CRM_HTTP"],0,3)=="yun") { ?>
	<input name="zzj" type="button" value="是总机" title="总机电话不能云呼叫" onClick="if (confirm('确认是总机电话吗？数据将做标示，云呼叫不会取到。')) {$.post('KH_saveinfo.php?lx=zongji', $('#lxxx').serialize(),function(data){alert('Data Loaded: ' + data);});CloseDiv('MyDiv','fade');}"> 
<?	}
 if ($state==-1) {?>
...
<br><input name="zxx" type="button" value="暂无意向" title="退回客户池" onClick="if (confirm('确认暂无意向吗？数据将退回客户池，下次联系1个月后。')) {$.post('KH_saveinfo.php?lx=tuihui', $('#lxxx').serialize(),function(data){alert('Data Loaded: ' + data);});CloseDiv('MyDiv','fade');}"> 
<? }?>
<br>成为客户，客户账号:<input name="zh" id="zh" type="text" value="<? echo $zh?>" style="width:120px">
...
<input name="zxx7" type="button" value="新建名片账号" onClick="window.open('http://mp.yikayin.com/newUser.php?uu=<? echo $_SESSION["YKOAUSER"]?>&cks=<? echo md5("hzyk".$_SESSION["YKOAUSER"]."winner")?>&khmc=<? echo urlencode($khmc)?>&lxr=<? echo urlencode($lxr)?>&mobile=<? echo $lxrmobile?>&lxdh=<? echo $lxdh;?>&lxdz=<? echo urlencode($lxdz)?>','HT_dhdj', 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=yes,copyhistory=yes,width=900,height=700,left=300,top=100'); return false">
