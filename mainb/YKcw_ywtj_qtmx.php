...
其他（非标类）订单明细（<? echo $_GET["d1"],"到",$_GET["d2"]?>）
	...
	$d1=$_GET["d1"];
	$d2=$_GET["d2"];
	$rs=mysql_query("select m.khmc,m.ddh,m.dje,ifnull(m.kdje,0),m.ddate,m.state,concat(m.xsbh,xm) xs from nc_erp.order_mainqt m,ry_xs where m.xsbh=ry_xs.xsbh and ddate>='$d1 00:00:00' and ddate<'$d2 00:00:00' and ssdq like '".$_GET["dq"]."' and (state='已收款' or state='待生产' or state='进入生产' or state='订单完成')",$conn);
	for ($i=0;$i<mysql_num_rows($rs);$i++) {
 ?>
        <tr>
			<td align="left" class="td_content" style='text-align:left'><? echo mysql_result($rs,$i,0),"[",mysql_result($rs,$i,6),"]";?></td>
            <td align="right" class="td_content" ><? echo mysql_result($rs,$i,2),"+",mysql_result($rs,$i,3);?></td>
            <td align="right" class="td_content" ><? echo '订单号：',mysql_result($rs,$i,1);?></td>
            <td class="td_content" align="right" ><? echo mysql_result($rs,$i,4);?></td>
            <td class="td_content" align="right" ><? echo mysql_result($rs,$i,5);?></td>
		</tr>
        <? $sr+=mysql_result($rs,$i,2)+mysql_result($rs,$i,3);
		} ?>
		 <tr>
