                ...
                <td valign="top">
                    <div style="padding:5px 32px 22px 55px; color:#58595B">
                        ...
                        <div>开始日期：
                            ...
                            <?
//                            $sql = "select m.hdczy,m.hdendtime,count(m.ddh) AS ddsl,r.xm,r.bh from b_ry r, order_mainqt m where r.dwdm='".$_SESSION["GDWDM"]."' and r.qx='hd' and locate(r.bh , m.hdczy)>0 and hdendtime>='$d1 00:00:00' and hdendtime<='$d2 23:59:59' group by m.hdczy order by m.hdendtime";
//                            $sql = "select m.hdczy,m.hdendtime,count(m.ddh) AS ddsl,r.xm,r.bh from b_ry r , order_mainqt m ,order_mxqt_hd h where r.dwdm='".$_SESSION["GDWDM"]."' and r.qx='hd' and locate(r.bh , m.hdczy)>0 and hdendtime>='$d1 00:00:00' and hdendtime<='$d2 23:59:59' group by m.hdczy order by m.hdendtime";
                            $sql = "select count(DISTINCT ddhao) AS ddsl,r.xm,r.bh,sum(h.sl*h.jg) as hdje from b_ry r , order_mxqt_hd h where r.dwdm in $dwdmStr and r.qx='hd' and locate(r.bh , h.hdczy)>0 and finishdate>='$d1 00:00:00' and finishdate<='$d2 23:59:59' group by r.xm";
                            ...
                            <table id="czytb" class="ptb" cellspacing="0" cellpadding="0" rules="all" border="0" id="gvOrder" style="border-width:0px;width:100%;border-collapse:collapse;border:1px solid #D3D3D3; border-bottom:0px;">
                                ...
                                <tr class="td_title" style="height:30px;">
                                    ...
                                    <th rowspan="2" align="center" scope="col">后加工总金额</th>
