...
//    }
    ...
    if($_GET['fpczy'] <> ''){
        ...
        $fpczys = " AND locate('$rspczybh',m.pczy)>0 ";
    }
...
//    总条数 统计数据
    if($_GET['minse'] <> '' || $_GET['maxse'] <> ''){
        ...
        $restotal = mysql_query("select count(*) as rowcount ,count(DISTINCT mx.ddh) as ddcount , sum(mx.sl1*mx.pnum1 + ifnull(mx.sl2*mx.pnum2,0)) as zps , sum(mx.sl1*mx.pnum1*mx.jg1 + ifnull(mx.sl2*mx.pnum2*mx.jg2,0)) as zddje from order_mxqt mx left join order_mainqt m on mx.ddh=m.ddh left join order_zh z on mx.ddh=z.ddh where fssj>='$d1 00:00:00' and fssj<='$d2 23:59:59' and z.khmc like '%{$d3}%'  and zy='订单结算' and zzfy in $dwdmStr $ddh $machines $materials $jes $prices $fpczys $ses",$conn);

    }else{
        $restotal = mysql_query("select count(*) as rowcount,count(DISTINCT mx.ddh) as ddcount , sum(mx.sl1*mx.pnum1 + ifnull(mx.sl2*mx.pnum2,0)) as zps , sum(mx.sl1*mx.pnum1*mx.jg1 + ifnull(mx.sl2*mx.pnum2*mx.jg2,0)) as zddje from order_mxqt mx left join order_mainqt m on mx.ddh=m.ddh left join order_zh z on mx.ddh=z.ddh where fssj>='$d1 00:00:00' and fssj<='$d2 23:59:59' and z.khmc like '%{$d3}%'  and zy='订单结算' and zzfy in $dwdmStr $ddh $machines $materials $jes $prices $fpczys ",$conn);
...
//    echo "select count(*) as rowcount , sum(mx.sl1*mx.pnum1 + ifnull(mx.sl2*mx.pnum2,0)) as zps , sum(mx.sl1*mx.pnum1*mx.jg1 + ifnull(mx.sl2*mx.pnum2*mx.jg2,0)) as zddje from order_mxqt mx left join order_mainqt m on mx.ddh=m.ddh left join order_zh z on mx.ddh=z.ddh where fssj>='$d1 00:00:00' and fssj<='$d2 23:59:59' and z.khmc like '%{$d3}%' and df>0 and zy='订单结算' and zzfy in $dwdmStr $ddh $machines $materials $jes $prices $fpczys $ses";
    ...
    if($_GET['minse'] <> '' || $_GET['maxse'] <> ''){
        ...
        $sql = "select mx.productname,mx.chicun , mx.sl, mx.n1,mx.machine1 , mx.paper1, mx.dsm1, mx.jldw1 , mx.pnum1,mx.sl1,mx.jg1,mx.n2, mx.machine2 , mx.paper2 , mx.dsm2 , mx.jldw2,mx.pnum2,mx.sl2,mx.jg2,z.*,m.pczy from order_mxqt mx left join order_mainqt m on mx.ddh=m.ddh left join order_zh z on mx.ddh=z.ddh where fssj>='$d1 00:00:00' and fssj<='$d2 23:59:59' and z.khmc like '%{$d3}%'  and zy='订单结算' and zzfy in $dwdmStr $ddh $machines $materials $jes $prices $fpczys $ses order by fssj limit $startrow ,$pagenum ";

    }else{

        $sql = "select mx.productname,mx.chicun , mx.sl, mx.n1,mx.machine1 , mx.paper1, mx.dsm1, mx.jldw1 , mx.pnum1,mx.sl1,mx.jg1,mx.n2, mx.machine2 , mx.paper2 , mx.dsm2 , mx.jldw2,mx.pnum2,mx.sl2,mx.jg2,z.*,m.pczy from order_mxqt mx left join order_mainqt m on mx.ddh=m.ddh left join order_zh z on mx.ddh=z.ddh where fssj>='$d1 00:00:00' and fssj<='$d2 23:59:59' and z.khmc like '%{$d3}%' and zy='订单结算' and zzfy in $dwdmStr $ddh $machines $materials $jes $prices $fpczys order by fssj limit $startrow ,$pagenum ";
    }
                                    ...
                                    <tr style="height:30px;">
                                        ...
                                        <td class="td_content">
                                            <select name="machine" style="height:29px;">
                                                ...
                                                <option value="Hp10000">Hp10000</option>
                                                <option value="Hp76000">Hp7600</option>
                                                <option value="Hp75000">Hp7500</option>
                                                <option value="Hp56000">Hp5600</option>
                                                <option value="奥西黑白">奥西黑白</option>
                                            </select>
<!--                                            <a href="excel_machine.php" target="_blank" class="detail">查看详情</a>-->
                                ...
                                if($_GET['machine']<>'') {
                                    ...
                                    if ($yq["machine1"] == $_GET['machine']) {
                                        ?>

                                        <tr style="height:30px">
                                            <td><a title="订单操作员" href="javascript:void(0);" onclick="window.open('dd_czy.php?ddh=<? echo $yq['ddh'] ?>','签单操作员','height=400px,width=620px,top=100px,left=200px,location=no,menubar=no,resizable=no,scrollbars=yes,status=no,titlebar=no,toolbar=no,directories=no');"><? echo $yq["ddh"]; // 订单号
                                                ?></a></td>
                                            <td><? echo $yq["fssj"];// 单据日期
                                                ?></td>
                                            <td><? echo $yq["khmc"];// 客户名称
                                                ?></td>
                                            <td><? echo $yq["machine1"];// 机型颜色
                                                ?></td>
                                    ...
                                    if ($yq["machine2"] == $_GET['machine']) {
                                        ?>

                                        <tr style="height:30px">
                                            <td><a title="订单操作员"  href="javascript:void(0);" onclick="window.open('dd_czy.php?ddh=<? echo $yq['ddh'] ?>','签单操作员','height=400px,width=620px,top=100px,left=200px,location=no,menubar=no,resizable=no,scrollbars=yes,status=no,titlebar=no,toolbar=no,directories=no');"><? echo $yq["ddh"]; // 订单号?></a></td>
