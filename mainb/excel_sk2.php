...
<form method="post" >
	按日期：<input onClick="WdatePicker();" maxlength="50" class="Wdate" style="cursor:pointer;" type="text" name="rq1" id="rq1" value="<? echo $d1;?>" size="9" readonly />～<input onClick="WdatePicker();" maxlength="50" class="Wdate" style="cursor:pointer;" type="text" name="rq2" id="rq2" value="<? echo $d2;?>" size="9" readonly />&nbsp;客户名称：<input type="text" name="fkhmc" width="15" value="<?echo $d3=="%"?"":$d3;?>"/>&nbsp;收款方式：<select name="skfs"><option value="">全部</option><option value="预存扣款"<?if($skfs=="预存扣款") echo " selected"?>>预存扣款</option><option value="现金"<?if($skfs=="现金") echo " selected"?>>现金</option><option value="支票"<?if($skfs=="支票") echo " selected"?>>支票</option><option value="POS刷卡" <?if($skfs=="POS刷卡") echo " selected"?>>POS刷卡</option><option value="汇款"<?if($skfs=="汇款") echo " selected"?>>汇款</option></select>
...
$rs = mysql_query("select m.ddh ddh,m.khmc khmc,m.dje dje,m.djje djje,z.df df,z.xsbh skfs,z.sksj sksj,m.skbz skbz from order_mainqt m left join order_zh z on m.ddh=z.ddh where z.fssj>='$d1 00:00:00' and z.fssj<='$d2 23:59:59' and m.khmc like '%{$d3}%' and state in ('待配送','订单完成') and zzfy='$dwdm' and z.zy='订单结算' and z.df>0 $skfstj",$conn);
if($_POST["bt2"]){
