...
<table width="90%" border="0" cellspacing="0" cellpadding="0" style="padding:0px;">
  <tbody><tr>
    <td valign="top">   
<div style="margin-left:20px"><br>新增用户（<? echo $_GET["d1"],"到",$_GET["d2"]?>）
	<table cellspacing="0" cellpadding="0" rules="all" border="1" id="gvOrder" style="border-width:0px;width:100%;border-collapse:collapse;border:1px solid #D3D3D3; border-bottom:0px;">
		<tbody><tr class="td_title" style="height:30px;">
			...
			<th width="14%" align="center" scope="col">所属销售</th>
			<th width="25%" align="center" scope="col">结账方式</th>
			<th width="17%" align="center" scope="col">注册时间</th>
			<th width="14%" align="center" scope="col">来源</th>
		</tr>
    <? $sr=0;
	
	$d1=$_GET["d1"];
	$d2=$_GET["d2"];
	$rs=mysql_query("select zh,depart,b.xm,b.xsbh,ry_xs.xm,payfs,zctime,registerfrom from base_user b,ry_xs where b.xsbh=ry_xs.xsbh and zctime>='$d1 00:00:00' and zctime<'$d2 00:00:00' and ssdq like '".$_GET["dq"]."' and qx>0",$conn);
	for ($i=0;$i<mysql_num_rows($rs);$i++) {
 ?>
        <tr>
			<td align="left" class="td_content" ><? echo mysql_result($rs,$i,0),"-",mysql_result($rs,$i,1),"-",mysql_result($rs,$i,2);?></td>
            <td align="right" class="td_content" ><? echo mysql_result($rs,$i,3),"-",mysql_result($rs,$i,4);?></td>
            <td align="right" class="td_content" ><? echo mysql_result($rs,$i,5);?></td>
            <td class="td_content" align="right" ><? echo mysql_result($rs,$i,6);?></td>
            <td class="td_content" align="right" ><? echo (mysql_result($rs,$i,7)=="")?"网站":mysql_result($rs,$i,7);?></td>
		</tr>
        <? $sr+=1;
		} ?>
		 <tr>
