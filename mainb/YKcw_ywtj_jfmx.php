...
<table width="90%" border="0" cellspacing="0" cellpadding="0" style="padding:0px;">
  <tbody><tr>
    <td valign="top">   
<div style="margin-left:20px"><br>收款明细（<? echo $_GET["d1"],"到",$_GET["d2"]?>）
	<table cellspacing="0" cellpadding="0" rules="all" border="1" id="gvOrder" style="border-width:0px;width:100%;border-collapse:collapse;border:1px solid #D3D3D3; border-bottom:0px;">
		<tbody><tr class="td_title" style="height:30px;">
			...
			<th width="14%" align="center" scope="col">收款金额</th>
	...
	$d1=$_GET["d1"];
	$d2=$_GET["d2"];
	$rs=mysql_query("select user_zh.*,depart from user_zh,base_user b,ry_xs where b.xsbh=ry_xs.xsbh and user_zh.zh=b.zh and fssj>='$d1 00:00:00' and fssj<'$d2 00:00:00' and ssdq='".$_GET["dq"]."' and jf<>0",$conn);
	for ($i=0;$i<mysql_num_rows($rs);$i++) {
 ?>
        <tr>
			<td align="left" class="td_content" ><? echo mysql_result($rs,$i,1),"-",mysql_result($rs,$i,"depart");?></td>
            <td align="right" class="td_content" ><? echo mysql_result($rs,$i,2);?></td>
